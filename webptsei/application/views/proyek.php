...
<body class=" font-inter dashcode-app" id="body_class">
    ...
    <main class="app-wrapper">
      ...
      overflow-hidden">
                                                    ...
                                                    <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700 data-table">
                                                        <thead class="bg-slate-200 dark:bg-slate-700">
                                                            <tr>
                                                                ...
                                                                <th scope="col" class="table-th">Nama Proyek</th>
                                                                <th scope="col" class="table-th">Client</th>
                                                                <th scope="col" class="table-th">Tanggal Mulai</th>
                                                                <th scope="col" class="table-th">Tanggal Selesai</th>
                                                                <th scope="col" class="table-th">Pemimpin Proyek</th>
                                                        ...
                                                        <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                                            ...
                                                            foreach ($data_proyek as $item): ?>
                                                                <tr>
                                                                    ...
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['namaProyek']); ?></td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['client']); ?></td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['tglMulai']); ?></td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['tglSelesai']); ?></td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['pimpinanProyek']); ?></td>
                                                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400"><?php echo htmlspecialchars($item['keterangan']); ?></td>
                                                                    ...
                                                                    <td class="table-td">
                                                                        <div class="flex space-x-3 rtl:space-x-reverse">
                                                                            <button class="action-btn" type="button" data-bs-toggle="modal" data-bs-target="#editProyekModal"
                                                                                data-id="<?php echo $item['id']; ?>"
                                                                                data-nama="<?php echo htmlspecialchars($item['namaProyek']); ?>"
                                                                                data-client="<?php echo htmlspecialchars($item['client']); ?>"
                                                                                data-tgl-mulai="<?php echo date('Y-m-d\TH:i', strtotime($item['tglMulai'])); ?>"
                                                                                data-tgl-selesai="<?php echo date('Y-m-d\TH:i', strtotime($item['tglSelesai'])); ?>"
                                                                                data-pimpinan="<?php echo htmlspecialchars($item['pimpinanProyek']); ?>"
            ...
            <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="editProyekModal" tabindex="-1" aria-hidden="true">
                    ...
                    <div class="modal-content border-none shadow-lg relative flex flex-col lg:w-[576px] w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
                        <div class="relative bg-white rounded-lg shadow dark:bg-slate-700">
                            ...
                            <div class="p-6 space-y-4">
                                <form id="proyekForm" class="flex flex-col space-y-3" action="<?php echo site_url('proyek/update'); ?>" method="POST">
                                    <input type="hidden" name="id" id="proyekId" value="">
                                    ...
                                    <div class="flex items-center justify-end rounded-b dark:border-slate-600">
                                        ...
                                        <button type="submit" class="btn btn-info" id="submitButton">Update Proyek</button>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            var editButtons = document.querySelectorAll('[data-bs-toggle="modal"][data-bs-target="#editProyekModal"]');

            editButtons.forEach(function(button) {
                button.addEventListener('click', function() {
                    var modal = document.querySelector('#editProyekModal');
                    var proyekId = button.getAttribute('data-id');
                    var namaProyek = button.getAttribute('data-nama');
                    ...
                    var pimpinanProyek = button.getAttribute('data-pimpinan');
                    var keterangan = button.getAttribute('data-keterangan');

                    modal.querySelector('#proyekId').value = proyekId;
                    modal.querySelector('#nama_proyek').value = namaProyek;
                    modal.querySelector('#client').value = client;
                    modal.querySelector('#tgl_mulai').value = tglMulai;
                    modal.querySelector('#tgl_selesai').value = tglSelesai;
                    modal.querySelector('#pimpinan_proyek').value = pimpinanProyek;
